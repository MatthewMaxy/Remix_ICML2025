...
from datasets.dataloader import AV_CD_Dataset, AV_CD_Dataset_Remix
...
def get_arguments():
    ...
    parser.add_argument('--log_path', default='log_model', type=str, help='path to save models')
    ...
    parser.add_argument('--gamma1', default=1, type=float, help='factor of video-loss in specific train')
    parser.add_argument('--gamma2', default=1, type=float, help='factor of audio-loss in specific train')
...
def remix(args, model, device, dataloader, epoch):
    audio_dataset, video_dataset = [], []
    softmax = nn.Softmax(dim=1)

    with torch.no_grad():
        model.eval()
        for step, (image, spec, label_index, label_name, url) in enumerate(tqdm(dataloader)):
            image = image.to(device)
            spec = spec.to(device)
            out, out_audio, out_video, a, v = model(spec.float(), image.float())
            pred_v = softmax(out_video)
            pred_a = softmax(out_audio)

            for i, item in enumerate(label_index):
                video_pred = pred_v[i].cpu().data.numpy()
                audio_pred = pred_a[i].cpu().data.numpy()
                KL_audio = kl_divergence(audio_pred, 6)
                KL_video = kl_divergence(video_pred, 6)
                if KL_audio < KL_video:
                    audio_dataset.append([url[i], label_name[i]])
                else:
                    video_dataset.append([url[i], label_name[i]])

    file_a = "./data/remix_a.csv"
    file_v = "./data/remix_v.csv"

    with open(file_a, 'w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
        for item in audio_dataset:
            writer.writerow([item[0], item[1]])

    with open(file_v, 'w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
        for item in video_dataset:
            writer.writerow([item[0], item[1]])

    audio_dataset = AV_CD_Dataset_Remix(modality='audio')
    video_dataset = AV_CD_Dataset_Remix(modality='video')
...
def train_epoch(args, epoch, model, device, dataloader, optimizer):
    ...
    beta = args.beta
    model.train()
    print(f"Epoch{epoch}: warm-up training... ")
    _loss = 0
...
def special_train_epoch(args, epoch, model, device, a_dataset, v_dataset, optimizer):
    ...
    len_v = len(v_dataset)
    if len_v > 0:
        v_dataloader = DataLoader(v_dataset, batch_size=args.batch_size, shuffle=True, num_workers=16, pin_memory=True)                             
        len_v = len(v_dataloader)
        for step, (image, spec, label, _, _) in enumerate(tqdm(v_dataloader)):
            optimizer.zero_grad()
            image = image.to(device)
            spec = spec.to(device)
            label = label.to(device)
            out, out_audio, out_video, a, v = model(spec.float(), image.float(), "video")
            loss_av = criterion(out, label)
            loss_v = criterion(out_video, label)
            loss = loss_av + gamma1 * loss_v
            loss.backward()
            optimizer.step()
            _loss += loss.item()
    else:
        print("Empty Video Subset!")

    len_a = len(a_dataset)
    if len_a > 0:
        a_dataloader = DataLoader(a_dataset, batch_size=args.batch_size, shuffle=True, num_workers=16, pin_memory=True) 
        len_a = len(a_dataloader)
        for step, (image, spec, label, _, _) in enumerate(tqdm(a_dataloader)):
            optimizer.zero_grad()
            image = image.to(device)
            spec = spec.to(device)
            label = label.to(device)
            out, out_audio, out_video, a, v = model(spec.float(), image.float(), "audio")
            loss_av = criterion(out, label)
            loss_a = criterion(out_audio, label)
            loss = loss_av + gamma2 * loss_a
            loss.backward()
            optimizer.step()
            _loss += loss.item()
    else:
        print("Empty Audio Subset!")
        
    return _loss / (len_a + len_v)
...
def valid(args, model, device, dataloader, epoch):
    ...
    with torch.no_grad():
        ...
        for step, (image, spec, label, _, url) in enumerate(tqdm(dataloader)):
            image = image.to(device)
            spec = spec.to(device)
            label = label.to(device)
...
def main():
    ...
    train_val_dataset = AV_CD_Dataset(mode='val')
    ...
    if args.train:
        ...
        for epoch in range(args.epochs):
            ...
            if epoch < args.warmup:
                ...
                a_dataset, v_dataset = remix(args, model, device, train_val_dataloader, epoch)
                batch_loss = special_train_epoch(args, epoch, model, device, a_dataset, v_dataset, optimizer)
                scheduler.step()
            
            acc= valid(args, model, device, val_dataloader, epoch)


            if acc > best_acc:
                best_acc = float(acc)

                model_name = 'Concat_with_Remix.pth'
        ...
        print(f"All training epoches of CREMAD REMIX finished, Best Acc:{best_acc}")
